...
class BusinessCRUD:
    ...
    def get_all(
        ...
        statement = (
            statement.offset(skip).limit(limit).order_by(Business.created_at.desc())
        )
    ...
    def get_by_type(
        ...
        statement = (
            select(Business)
            .where(Business.type == business_type)
            .offset(skip)
            .limit(limit)
            .order_by(Business.created_at.desc())
        )
    ...
    def search_by_name(
        ...
        statement = (
            select(Business)
            .where(Business.name.ilike(f"%{name}%"))
            .offset(skip)
            .limit(limit)
            .order_by(Business.created_at.desc())
        )
    ...
    def search_by_location(
        ...
        statement = (
            select(Business)
            .where(Business.address.ilike(f"%{location}%"))
            .offset(skip)
            .limit(limit)
            .order_by(Business.created_at.desc())
        )
